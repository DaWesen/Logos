...
func checkGlobalRateLimit(c *gin.Context, ip, path, method, userID string, cache cache.Cache) bool {
	...
	if err != nil {
		logger.Warn("全局限流检查失败，放行请求",
			logger.ErrorField(err))
		return true
	}

	if !allowed {
		logger.Warn("触发全局API限流",
			logger.StringField("ip", ip),
			logger.StringField("path", path),
			logger.StringField("method", method))

		c.JSON(http.StatusTooManyRequests, model.Error(429, "服务器繁忙，请稍后再试"))
		c.Abort()
		return false
	}
...
func checkUserRateLimit(c *gin.Context, userID, path string, cache cache.Cache) bool {
	...
	if !allowed {
		...
		c.JSON(http.StatusTooManyRequests, model.Error(429, "操作过于频繁，请稍后再试"))
		c.Abort()
		return false
	}
...
func IPBasedRateLimit(cache cache.Cache, requests int, window time.Duration) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !allowed {
			logger.Warn("IP访问频率超限",
				logger.StringField("ip", ip),
				logger.IntField("requests", requests))

			c.JSON(http.StatusTooManyRequests, model.Error(429, "您的访问过于频繁，请稍后再试"))
			c.Abort()
			return
		}
...
func ConcurrentRequestLimit(cache cache.Cache, maxConcurrent int, timeout time.Duration) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		case semaphore <- struct{}{}:
			...
			c.Next()
		default:
			logger.Warn("并发请求数超过限制")

			c.JSON(http.StatusServiceUnavailable, model.Error(503, "系统繁忙，请稍后重试"))
			c.Abort()
		}
	}
}
...
func BurstProtection(cache cache.Cache, threshold int, cooldown time.Duration) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if current > int64(threshold) {
			...
			c.JSON(http.StatusForbidden, model.Error(403, "您的访问已被临时限制"))
			c.Abort()
			return
		}
