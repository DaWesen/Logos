...
func (s *userServiceImpl) checkLoginRateLimit(ctx context.Context, account string) error {
	...
	if current > 5 {
		...
		return fmt.Errorf("操作过于频繁，请%d秒后再试", 60)
	}

	return nil
}

func (s *userServiceImpl) checkRegisterRateLimit(ctx context.Context, username string) error {
	if s.cache == nil {
		return nil
	}

	key := fmt.Sprintf("ratelimit:register:%s", username)

	current, err := s.cache.IncrBy(ctx, key, 1)
	if err != nil {
		logger.Warn("注册限流计数失败，放行",
			logger.ErrorField(err))
		return nil
	}
